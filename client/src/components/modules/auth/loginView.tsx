    ...
    <main className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-gray-900 rounded-2xl shadow-xl shadow-purple-950 border border-purple-800 p-8 flex flex-col gap-6">
        ...
        <div className="text-center">
          ...
          <p className="text-indigo600 text-sm mt-1">Acesso ao painel administrativo</p>
        </div>

        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label className="text-indigo-600 text-sm font-semibold">Email</label>
            <input
              type="email"
              value={email}
              onChange={e => setEmail(e.target.value)}
              placeholder="seu@email.com"
              className="bg-gray-800 text-white placeholder-gray-500 border border-gray-700 focus:border-indigo-500 focus:outline-none rounded-lg px-4 py-3 text-sm transition-colors"
            />
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-indigo-500 text-sm font-semibold">Senha</label>
            <input
              type="password"
              value={senha}
          ...
          <button
            type="submit"
            className="mt-2 bg-indigo-700 hover:bg-indigo-600 active:bg-indigo-800 text-white font-bold py-3 rounded-lg transition-colors cursor-pointer tracking-wide"
          >
            Entrar
          </button>
        </form>
